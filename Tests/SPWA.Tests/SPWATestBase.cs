...
namespace SPWA.Tests
{
    public abstract class SPWATestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<SPWADbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<SPWADbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SPWADbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SPWADbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SPWADbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SPWADbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<SPWADbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SPWADbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SPWADbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SPWADbContext, Task<T>> func)
        {
            T result;
